...
**Recipes → Catalog** is the shared entry point. Each Git repository has one page with **Configuration**, **Updates**, and **Devices**, plus retained immutable versions. Package-only recipes use the same editor. Unfinished and failed editable work stays separate from saved recipes.

**Add from GitHub** pins and inspects source without executing repository scripts or starting a model. **Help fix this configuration** creates independent editable work from a saved package, even when its original repository or earlier editing document is unavailable. Updates retain the saved baseline and compare pinned source revisions, configuration, permissions, and helper files.

AI assistance is optional. Configure a local model, OpenAI-compatible provider, or Codex connection under **Settings**. Select bounded source evidence or failure logs, review the exact request and destination, then consent explicitly. Suggested changes require a separate review before they affect editable work. They never modify the saved recipe or running models automatically. Independent raw JSON, structured fields, helper text, context selections, and non-secret provider inputs have explicit browser recovery; restoring never replays an operation.

**Add to library** and **Save updated recipe** save only the reviewed immutable configuration and helper package. They do not download resources to devices, change running versions, or launch containers. Previously saved versions remain addressable, including distinct reviewed packages from the same upstream commit.
...
Model workloads run without container-imposed CPU quotas, CPU affinity restrictions, or RAM ceilings. Recipes do not expose or accept these controls. Device inventory and hardware-compatibility checks remain separate from runtime resource restrictions.

To change an existing running model, select its exact deployment under **Replace selected running versions**, review the target already saved in the catalog, and confirm the interruption. Unselected devices and deployments are not replaced. Failure attempts restoration of the selected old running versions, but restoration is not guaranteed and never rolls back the saved catalog version.

### GLM-5.3 Flash on two DGX Sparks

The Library has deterministic compilers for the EXL3 target and [`tonyd2wild/GLM-5.3-Flash-NVFP4-DFlash2-2x-DGX-Spark`](https://github.com/tonyd2wild/GLM-5.3-Flash-NVFP4-DFlash2-2x-DGX-Spark). Choose **Add from GitHub**, paste the repository URL, optionally pin a full commit, and review and save the generated immutable contract before downloading or launching. LMW treats the repository as data and does not execute its scripts during import.
