...
func (f *fakeSource) List(context.Context) ([]deploy.Deployment, error) {
	return f.deps, nil
}
...
func createDeploymentRow(t *testing.T, database *sql.DB, id string) {
	t.Helper()
	ctx := context.Background()
	if _, err := database.ExecContext(ctx, `INSERT OR IGNORE INTO recipes (digest, name, version, trust_state, manifest) VALUES (?,?,?,?,?)`, "r1", "r", "1", "verified", "{}"); err != nil {
		t.Fatal(err)
	}
...
func TestCollectFiltersAndPersists(t *testing.T) {
	...
	source := &fakeSource{deps: []deploy.Deployment{
		...
		{ID: "d-preparing", DesiredState: "running", ObservedState: "preparing", Endpoint: &deploy.Endpoint{Host: "127.0.0.1", Port: int32(portOf(srv.URL))}},
		{ID: "d-noendpoint", DesiredState: "running", ObservedState: "healthy", Endpoint: nil},
	}}
	...
	for _, skipped := range []string{"d-noendpoint", "d-preparing"} {
