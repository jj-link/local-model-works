...
func TestDoSendsExactIdentityAndWaitsForAck(t *testing.T) {
	...
	go func() {
		result, err := client.Do(context.Background(), agentv1.WorkloadOp_WORKLOAD_OP_INSPECT, nil, time.Second)
		done <- result
		errs <- err
	}()
	message := <-connection.SendCh
	command := message.GetWorkloadCommand()
	if command == nil || command.GetRunId() != "run-1" || command.GetDeploymentId() != "" || command.GetRank() != 0 || command.GetOp() != agentv1.WorkloadOp_WORKLOAD_OP_INSPECT {
		t.Fatalf("command = %+v", command)
	}
	broker.Deliver(&agentv1.CommandResult{CommandId: command.GetCommandId(), Ok: true, ContainerState: "running"})
	if err := <-errs; err != nil {
		t.Fatal(err)
	}
	if result := <-done; !result.GetOk() || result.GetContainerState() != "running" {
