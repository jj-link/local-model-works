        ...
        <div className="text-right font-mono text-[10px] text-faint">
          {node.status === "pending" ? (
            ...
            <span className="text-ok">fresh</span>
                        ...
                        <span>sample {relativeTime(sampleAge(sample), now)}</span>
          )}
        </div>
      </header>
...
// Sample age tracks the telemetry sample timestamp, not the heartbeat, so a
// node can keep sending heartbeats while its latest metric sample is stale.
function sampleAge(sample: NodeTelemetrySample | undefined): string | null {
  if (!sample?.ts) return null;
  return new Date(sample.ts * 1000).toISOString();
}
