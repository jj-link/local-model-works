...
var purposes = map[string]bool{"huggingface": true, "github": true, "registry": true, "model-provider": true, "runtime-provider": true}
...
func (m *Module) putSecret(w http.ResponseWriter, r *http.Request) {
	...
	if !purposes[body.Purpose] {
		httpx.WriteErr(w, http.StatusUnprocessableEntity, "resource.unprocessable",
			"purpose must be one of huggingface, github, registry, model-provider, runtime-provider")
		return
	}
