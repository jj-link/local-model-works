...
var purposes = map[string]bool{"huggingface": true, "github": true, "registry": true, "recipe-assistant": true}
...
func (m *Module) putSecret(w http.ResponseWriter, r *http.Request) {
	...
	if !purposes[body.Purpose] {
		httpx.WriteErr(w, http.StatusUnprocessableEntity, "resource.unprocessable",
			"purpose must be one of huggingface, github, registry, recipe-assistant")
		return
	}
