...
- Fix: refresh recipe and deployment state when an update run finishes, clear stale update actions, and show the installed source commit so commit-only updates remain visible when an upstream recipe keeps the same version label.
